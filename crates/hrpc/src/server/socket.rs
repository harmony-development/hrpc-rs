...
mod internal {
    ...
    type SenderChanWithReq<Req> = (
        Result<Req, HrpcError>,
        oneshot::Sender<Result<(), HrpcError>>,
    );
        ...
        Resp: PbMsg + Default + 'static,
    {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    tokio::select! {
                        ...
                        Some((req_res, chan)) = send_msg_rx.recv() => {
                            let data = match req_res {
                                Ok(req) => encode_message(&mut buf, &req),
                                Err(err) => encode_hrpc_error(&mut buf, &err),
                            };
        ...
        pub async fn send_message(&self, req: Req) -> Result<(), HrpcError> {
            ...
            if self.is_closed() || self.tx.send((Ok(req), req_tx)).await.is_err() {
                Err(SocketError::AlreadyClosed.into())
            } else {
                req_rx
                    .await
        ...
        /// ## Notes
        /// This will block if the inner send buffer is filled.
        pub async fn send_error(&self, err: HrpcError) -> Result<(), HrpcError> {
            let (req_tx, req_rx) = oneshot::channel();
            if self.is_closed() || self.tx.send((Err(err), req_tx)).await.is_err() {
                Err(SocketError::AlreadyClosed.into())
            } else {
                req_rx
                    .await
