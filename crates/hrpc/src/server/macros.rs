...
/// Macro to workaround an issue that causes plain `async fn` handlers to not be used
/// on `set_something_handler` methods generated by `hrpc-build`.
/// use macro_rules_attribute::apply;
///
///
/// You can use this in conjunction with the `macro_rules_attribute` crate,
/// which will allow you to write code like this:
/// ```rust,ignore
/// use macro_rules_attribute::apply;
/// use hrpc::{handler, server::prelude::*};
///
/// struct Service;
///
/// // impl SomethingService for Service
///
/// #[apply(handler!)]
/// async fn something(svc: &mut Service, request: Request<ReqType>) -> ServerResult<Response<RespType>> {
///     // ...
/// }
///
/// async fn main() {
///     SomethingServer::new(Service)
///         .set_something_handler(something)
...
macro_rules! handler {
    (
        ...
        $( #[$attr] )*
        $pub
        fn $fname ( $($args)* ) -> impl ::std::future::Future<Output = ($($Ret)?)> {
            async move { $($body)* }
        }
    };
}
